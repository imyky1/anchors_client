...
function Editworkshop(props) {
  ...
  useEffect(() => {
    ...
    const formatDate = new Date(workshopInfo?.startDate);
    const defaultValue = formatDate.toLocaleDateString("en-CA");
    setdata({
      sname: workshopInfo?.sname,
      sdesc: workshopInfo?.sdesc,
      smrp: workshopInfo?.smrp,
      ssp: workshopInfo?.ssp,
      startDate: defaultValue,
      maxCapacity: workshopInfo?.maxCapacity,
      svideo: workshopInfo?.svideo,
      meetlink: workshopInfo?.meetlink,
    });
    setTags(workshopInfo?.tags);
    setContent(workshopInfo?.ldesc);
    setTime({
      startTime: workshopInfo?.time?.startTime,
      endTime: workshopInfo?.time?.endTime,
    });
    if (workshopInfo?.afterstartentry?.allowed === false) {
      setAfterStartEntry({
        allowed: false,
        discount: "",
      });
