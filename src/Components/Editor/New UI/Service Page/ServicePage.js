...
export const ServiceCards = ({ sname, simg, slug, stype }) => {
  ...
  return (
    <div className="new_service_page_service_card">
      <img src={simg} alt="" />
      <h3>{sname.length > (window.screen.width > 600 ? 45 : 30) ? sname.slice(0,(window.screen.width > 600 ? 45 : 30)) + "..." : sname}</h3>
        ...
        <div>
          ...
          <p>
            {Array(rating)
              .fill("a")
              ?.map((e, i) => {
                return <RiStarSFill size={window.screen.width > 600 ? 16 : 12} color="rgba(255, 214, 0, 1)" />;
              })}
          </p>
        </div>
      </section>
                ...
                <section className="action_points_new_service_page">
                  ...
                  <div>
                    ...
                    <button onClick={()=>{
                      ...
                      window.open(`https://api.whatsapp.com/send?text=Checkout this Important resource -- *${serviceInfo?.service?.sname}* at https://www.anchors.in/s/${slug}?utm_medium=whatsapp&utm_source=wahtsapp&utm_campaign=company-question`)
                    }}>
                      <BsWhatsapp /> Share
                    </button>
                  </div>
