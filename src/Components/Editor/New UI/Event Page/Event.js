...
//           setPaymentProcessing(true);
//           setLoader(true);
//           toast.info("Payment not yet completed, Complete the payment first", {
//             position: "top-center",
//             autoClose: 3500,
//           });
//           setTimeout(() => {
...
//                   {convertTime(eventInfo?.event?.time?.endTime)} (IST)
//                 </span>
//               </section>
//               {!eventFinished && (
//                 <section className={`scrollable_section_event`}>
    ...
    <>
      ...
      <div className="event_page_outside_wrapper">
        ...
        <div className="hoc2_wrraper_event_page">
          ...
          {!alreadyOrderPlaced && !eventFinished && (
            <section className="benefits_event_page_wrapper">
              ...
              Register & unlock your Unique Referral Code to avail AMAZING perks.
              </span>
          ...
          {feedbacks?.filter((e, i) => {
            ...
            <div className="hoster_details_event_page">
              ...
              <section>
               {eventInfo?.event?.c_id?.email && <div>
                  <CiMail />
                  {eventInfo?.event?.c_id?.email}
                </div>}
                {eventInfo?.creator?.phone && <div>
                  <MdLocalPhone />
                  {eventInfo?.creator?.phone}
                </div>}
              </section>

              <div>
                {eventInfo?.creator?.linkedInLink && <CiLinkedin onClick={()=>{window.open(eventInfo?.creator?.linkedInLink); mixpanel.track("Host linkedin link clicked")}}/>}
                {eventInfo?.creator?.instaLink && <IoLogoInstagram onClick={()=>{window.open(eventInfo?.creator?.instaLink); mixpanel.track("Host insta link clicked")}}/>}
                {eventInfo?.creator?.twitterLink && <FaXTwitter onClick={()=>{window.open(eventInfo?.creator?.twitterLink); mixpanel.track("Host twitter link clicked")}}/>}
                {eventInfo?.creator?.teleLink && <RiTelegramLine onClick={()=>{window.open(eventInfo?.creator?.teleLink); mixpanel.track("Host telegram link clicked")}}/>}
                {eventInfo?.creator?.fbLink && <CiFacebook onClick={()=>{window.open(eventInfo?.creator?.fbLink); mixpanel.track("Host facebook link clicked")}}/>}
              </div>
            </div>
          </section>
        </div>
        ...
        {isVisibleFloater && !eventFinished && (
          ...
          >
            <span>
              {alreadyOrderPlaced ? "Get Your Attendee Card" : paymentProcessing ? "Processing..." : "Register for Event"}
            </span>
            <div>
              {!alreadyOrderPlaced && (eventInfo?.event?.isPaid ? (
                <>
                  ₹{eventInfo?.event?.ssp} <span>{eventInfo?.event?.smrp}</span>
                </>
              ) : (
                "For Free"
              ))}
